...
function setup_fortio_and_prometheus() {
    ...
    local report_port="$(kubectl get services -n "${NAMESPACE}" fortioclient -o jsonpath="{.spec.ports[6].nodePort}")"
...
# Start run perf test
echo "Start to run perf benchmark test, all collected data will be dumped to ..."
...
read_perf_test_conf "${WD}/configs/run_perf_test_1.conf"
